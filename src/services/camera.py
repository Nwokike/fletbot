...
Returns raw image bytes for multimodal AI input.

API verified against flet-camera v0.84.0:
- Camera(on_state_change=...)
- initialize(description, resolution_preset, ...)
    ...
    async def capture_photo(self) -> tuple[bytes, str] | None:
        ...
        if not _HAS_CAMERA:
            ...
            self._page.show_dialog(
                ft.SnackBar(content=ft.Text("Camera is not available on desktop"))
            )
            return None
        ...
        try:
            ...
            if not cameras:
                self._page.show_dialog(
                    ft.SnackBar(content=ft.Text("No cameras found"))
                )
                return None
            ...
            self._page.show_dialog(
                ft.SnackBar(content=ft.Text(f"Camera error: {e}"))
            )
        finally:
            # Cleanup
            if camera:
